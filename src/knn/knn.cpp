...
{
	int gray = 0;
	FGMask.create(IMG_HGT, IMG_WID, CV_8UC1);// 输出图像初始化
	...
	for (int f = 0; f < solid_frame; f++)
	{
		...
		for (int i = 0; i < IMG_HGT; i++)
		{
			for (int j = 0; j < IMG_WID; j++)
			{
				...
				framePixelHistory[i*IMG_WID + j].IsBG[f] = 1;// 当前点作为背景点存入历史信息
			}
		}
	}
...
void KNN_BGS::knn_core()
{
	...
	for (int i = 0; i < IMG_HGT; i++)
	{
		for (int j = 0; j < IMG_WID; j++)
		{
			...
			{
				if (fabs((float)gray - framePixelHistory[i*IMG_WID + j].gray[n]) < defaultDist2Threshold)// 灰度差别是否位于设定阈值内
				{
					fit++;
					if (framePixelHistory[i*IMG_WID + j].IsBG[n])// 历史信息对应点之前被判断为背景
					{
						fit_bg++;
					}
				}
			}
			if (fit_bg >= knnv)// 当前点判断为背景
			{
			...
			framePixelHistory[i*IMG_WID + j].IsBG[index] = fit >= knnv ? 1 : 0;// 当前点作为背景点存入历史信息
		}
	}
	frameCnt++;
}
...
{
	return p1.width * p1.height > p2.width * p2.height;//降序排列  
}
...
void KNN_BGS::processRects()
{
	...
	findContours(FGMask.clone(), contours, hierarcy, CV_RETR_EXTERNAL, CHAIN_APPROX_NONE); //查找轮廓
